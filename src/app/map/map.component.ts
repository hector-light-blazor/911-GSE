...
export class MapComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //control map...
    this._appService.contrlMap.takeWhile(() => this.isAlive).subscribe(value => {
        console.log(value);
  ...
  onEnterSearch(){
    ...
    this._appService.GET_METHOD("search/geocoder/?address=" + this.searchStr, "GEOCODE").subscribe(response => {
              ...
              setTimeout(function(){
                for(var i = 0; i < self.emsArr.length; i++){
                  if(self.emsArr[i].geometry.contains(evt.mapPoint)){
                      document.getElementById("medical").innerHTML = self.emsArr[i].attributes.dispname;
            ...
            setTimeout(function(){
                for(var i = 0; i < self.fireArr.length; i++){
                  if(self.fireArr[i].geometry.contains(evt.mapPoint)){
                      document.getElementById("fire").innerHTML = self.fireArr[i].attributes.dispname;
            ...
            setTimeout(function(){
                for(var i = 0; i < self.lawArr.length; i++){
                  if(self.lawArr[i].geometry.contains(evt.mapPoint)){
                      document.getElementById("law").innerHTML = self.lawArr[i].attributes.dispname;
  ...
  getDPSTransfer(name:string):void{
    ...
    this._appService.GET_METHOD("alispill/" + name, "MAIN").subscribe(response => {
  ...
  getDPSTransferLoop(name:string){
    ...
    setInterval(function(){
        ...
        self._appService.GET_METHOD("alispill/" + name, "MAIN").subscribe(response => {
